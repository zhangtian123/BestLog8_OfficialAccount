				        ...
				        userInfo = data;
				          Userwcf.Usercheck1(userInfo.userid,null,null,"BestFMS8",0,function(result){
							...
							else
							{
								...
								{
									res.render('personalProfileForManager',{ title: 'BestLOG8',username:req.session.user.USERCODE,showname:req.session.user.SHOWNAME,flag:req.session.user.FLAG,openid:req.session.user.OPENID});
								}
								else if(result.USERFLAG=="SUPPLIER")
								{
									res.render('personalProfileForThird',{ title: 'BestLOG8',username:req.session.user.USERCODE,showname:req.session.user.SHOWNAME,flag:req.session.user.FLAG,openid:req.session.user.OPENID});
								}
								else if(result.USERFLAG == "DECLARATION")
								{
									res.render('personalProfileForDeclaration',{ title: 'BestLOG8',username:req.session.user.USERCODE,showname:req.session.user.SHOWNAME,flag:req.session.user.FLAG,openid:req.session.user.OPENID});
								}
								else if(result.USERFLAG == "TRAILER")
								{
									res.render('personalProfileForTrailer',{ title: 'BestLOG8',username:req.session.user.USERCODE,showname:req.session.user.SHOWNAME,flag:req.session.user.FLAG,openid:req.session.user.OPENID});
								}
								else
								{
									res.render('personalProfileForClient',{ title: 'BestLOG8',username:req.session.user.USERCODE,showname:req.session.user.SHOWNAME,flag:req.session.user.FLAG,openid:req.session.user.OPENID});
								}
							}
						})
				      }
...
router.get('/remittance', function(req, res, next) {
	if(req.session.user){
		res.render('remittance', { title: 'BestLOG8' });
	}
  else
  {
  	res.render('loginMain', { title: 'BestLOG8' });
...
router.get('/billaudit', function(req, res, next) {
	if(req.session.user){
		res.render('billaudit', { title: 'BestLOG8' });
	}
  else
  {
  	res.render('loginMain', { title: 'BestLOG8' });
...
router.get('/billconfirm', function(req, res, next) {
	if(req.session.user){
		res.render('billconfirm', { title: 'BestLOG8' });
	}
  else
  {
  	res.render('loginMain', { title: 'BestLOG8' });
...
router.get('/registerMain', function(req, res, next) {
	res.render('registerMain', { title: 'BestLOG8' });
});

/* GET 注册设置密码页. */
router.get('/registerSetPw', function(req, res, next) {
	res.render('registerSetPw', { title: 'BestLOG8' });
});

/* GET 注册填写公司信息页. */
router.get('/registerCominfo', function(req, res, next) {
	res.render('registerCominfo', { title: 'BestLOG8' });
});

/* GET 注册填写公司信息页. */
router.get('/takephoto', function(req, res, next) {
	res.render('takephoto', { title: 'BestLOG8' });
});

/* GET 注册填写完成. */
router.get('/registerWait', function(req, res, next) {
	res.render('registerWait', { title: 'BestLOG8' });
});

/* GET 注册审核. */
router.get('/registerAudit', function(req, res, next) {
	if(req.session.user){
		res.render('registerAuditing', { title: 'BestLOG8' });
	}
  else
  {
  	res.render('loginMain', { title: 'BestLOG8' });
...
router.get('/registerDetail', function(req, res, next) {
	res.render('registerDetail', { title: 'BestLOG8',registeruser:req.session.register.registeruser,taxnumber:req.session.register.taxnumber,
	COMPANY:req.session.register.COMPANY,COMPANYCN:req.session.register.COMPANYCN,REGADDRESS:req.session.register.REGADDRESS,
	REGADDRESSEN:req.session.register.REGADDRESSEN,EMAIL:req.session.register.EMAIL,BILLID:req.session.register.BILLID,STORAGEPATH:req.session.register.STORAGEPATH });
});
...
router.get('/leaveAudit', function(req, res, next) {
	if(req.session.user){
		res.render('leaveAudit', { title: 'BestLOG8' });
	}
  else
  {
  	res.render('loginMain', { title: 'BestLOG8' });
...
router.get('/loginForgetPw', function(req, res, next) {
	res.render('loginForgetPw', { title: 'BestLOG8' });
});

/* GET 重设密码. */
router.get('/loginSetPw', function(req, res, next) {
	res.render('loginSetPw', { title: 'BestLOG8' });
});
...
//	if(req.session.user){
		Freight.getPackage("1==1","",5,function(result){
			res.render('freightQuery', { title: 'BestLOG8',packagedata:JSON.stringify(result),isuser:(req.session.user.USERFLAG=="USER" ? true : false)});
		})
//	}
//else
//{
...
router.get('/renderQuote',function(req,res,next){
	if(req.session.user.USERFLAG=='USER'){
		res.render('renderQuote', { title: 'BestLOG8',id:req.query.id});
	}
})
...
router.get('/stateback1', function(req, res, next) {
	if(req.session.user){
		res.render('statusBrokerMain', { title: 'BestLOG8' });
	}
  else
  {
  	res.render('loginMain', { title: 'BestLOG8' });
...
router.get('/statusBrokerOrdertaking', function(req, res, next) {
	res.render('statusBrokerOrdertaking', { title: 'BestLOG8' });
});

/* GET 状态反馈之申报. */
router.get('/statusBrokerDoing', function(req, res, next) {
	res.render('statusBrokerDoing', { title: 'BestLOG8' });
});

/* GET 状态反馈之放行. */
router.get('/statusBrokerClearance', function(req, res, next) {
	res.render('statusBrokerClearance', { title: 'BestLOG8' });
});

/* GET 状态反馈. */
router.get('/stateback2', function(req, res, next) {
	if(req.session.user){
		res.render('statusTrailerMain', { title: 'BestLOG8' });
	}
  else
  {
  	res.render('loginMain', { title: 'BestLOG8' });
...
router.post('/profile', upload.single('avatar'), function(req, res, next) {
	...
	var userstr = {COMCONTACTID:guid1,CONTACTCOMGUID:guid2,ONLINEID:req.session.user.USERCODE,IDPASSWORD:req.session.user.PASSWORD,MOBILEPHONE:req.session.user.MOBILENO,REGDATE:Time.getNowDate(0,0,0),RowState:"4"}
  ...
  var arrayphoto = [];
  var DocumentList = {DOCUMENTID:guid3,FILENAME:filename,STORAGEPATH:filepath,RowState:"4"};
...
router.get('/getRemittanceList', function(req, res, next) {
  var parameter = req.query;
	...
	console.log(predicate);console.log(time);
...
router.post('/statusBroker', function(req, res, next) {
    ...
    form.parse(req, function(err, fields, files) {
        ...
        for(item in files){
            ...
            var DocumentList = {DOCUMENTID:guid3,FILENAME:files[item].name,STORAGEPATH:"/"+billCode+"/"+billNo+"/"+files[item].name,RowState:"4"};
            arrayphoto.push(DocumentList);
        }
        /*同时还要上传FTP*/
       var cc = new Client();
...
router.post('/bindingwx', function(req, res, next) {
	...
	if(req.session.user)
	{
		...
		Userwcf.ChangeOpenId(userId,openId,userFlag,function(result){
			...
			res.send({result:result,USERID:userId,USERFLAG:userFlag,OPENID:openId});
		})
	}
});

router.get('/downloadapp', function(req, res, next) {
		res.render('downloadapp', { title: 'BestLOG8'});
});
...
router.get('/getWXCode', function(req, res, next) {
  res.render('getWXCode', { title: 'BestLOG8'});
});

router.get('/orderStatistics', function(req, res, next) {
	res.render('orderStatistics', { title: 'BestLOG8' });
});
...
router.get('/customerStatistics',function(req,res,next){
	res.render('customerStatistics',{title:'BestLOG8'});
});
...
router.get('/shareStatusBack',function(req,res,next){
	...
	Order.ValidOcShareLog(parameter.sid,48,function(result){
		if(result==1){
			...
			if(type=="bg"){
				res.render('shareStatusBackByBG', { ids: parameter.ids });
			}
			else if(type=="tc"){
				res.render('shareStatusBackByTC', { ids: parameter.ids});
			}
			else if(type=="bill"){
				res.render('shareStatusBackByBill', { ids: parameter.ids,display:imgShow ? 'display' : 'none',tooltips:(imgShow ? '账单确认结果有效期为48小时，更多服务请关注公众号' : '分享内容已经生成，有效期为48小时，请点右上角...分享') });
			}
			else if(type=="freight"){
				res.render('shareStatusBackByFreight', { ids: parameter.ids,display:imgShow ? 'display' : 'none',tooltips:(imgShow ? '报价结果有效期为48小时，更多服务请关注公众号' : '分享内容已经生成，有效期为48小时，请点右上角...分享') });
			}
			else if(type=="quote"){
				res.render('shareStatusBackByQuote', { ids: parameter.ids,display:imgShow ? 'display' : 'none',tooltips:(imgShow ? '报价结果有效期为48小时，更多服务请关注公众号' : '分享内容已经生成，有效期为48小时，请点右上角...分享') });
			}
			else{//order
				res.render('shareStatusBackByOrder', { ids: parameter.ids,display:imgShow ? 'display' : 'none',tooltips:(imgShow ? '状态返馈结果有效期为48小时，更多服务请关注公众号' : '分享内容已经生成，有效期为48小时，请点右上角...分享') });
			}
		}
		else{
...
router.get('/setShareLogBySystem',function(req,res,next){
	Order.SetOcShareLog(function(result){
		var parameter = req.query;
	    var type=parameter.type;
	    res.render('setShareLogBySystem',{type:type,ids:parameter.ids,sid:result});
...
router.get('/placeShippingOrder', function(req, res, next) {
	res.render('placeOrder', { title: 'BestLOG8',showname:req.session.user==null?"": req.session.user.SHOWNAME });
});
...
router.get('/shippingOrder', function(req, res, next) {
	if(req.session.user){
		res.render('orderList', { title: 'BestLOG8' });
	}
  else
  {
  	res.render('loginMain', { title: 'BestLOG8' });
